...
R --vanilla << EOF
    ...
    Massdta <- Massdta[!is.na(Massdta[,9]),]

    #change colnames of first covariates
    colnames(Massdta)[1] = "id"

    #add animal column
    Massdta[,10] = Massdta[,1]
    colnames(Massdta)[10] = "animal"

    #set factor levels with only observations we have
    Massdta[,1] = factor(Massdta[,1], levels = unique(as.character(Massdta[,1]))[!is.na(unique(as.character(Massdta[,1])))])
    Massdta[,2] = factor(Massdta[,2], levels = unique(as.character(Massdta[,2]))[!is.na(unique(as.character(Massdta[,2])))])
    Massdta[,6] = factor(Massdta[,6], levels = unique(as.character(Massdta[,6]))[!is.na(unique(as.character(Massdta[,6])))])
    Massdta[,7] = factor(Massdta[,7], levels = unique(as.character(Massdta[,7]))[!is.na(unique(as.character(Massdta[,7])))])

    # as data frame
    Massdta <- as.data.frame(Massdta)
    ...
    Massdta = Massdta[Massdta[,4] < 1000,]

    #susbsample individuals in the pedigree
    Massdta = Massdta[Massdta[,10] %in% p2[,1],]
    ...
    save.image("./5_ID_Models/5.2_modelsOutput/AM_Mass_ADULTS_noRank.RData")
