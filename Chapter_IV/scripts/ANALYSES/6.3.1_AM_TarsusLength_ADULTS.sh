...
R --vanilla << EOF
    ...
    Tarsuslengthdta <- Tarsuslengthdta[!is.na(Tarsuslengthdta[,8]),]

    #change INDV colnames
    colnames(Tarsuslengthdta)[1] = "id"
    #add anumal column
    Tarsuslengthdta[,9] = Tarsuslengthdta[,1]
    colnames(Tarsuslengthdta)[9] = "animal"

    #set factor levels with only observations we have
    Tarsuslengthdta[,1] = factor(Tarsuslengthdta[,1], levels = unique(as.character(Tarsuslengthdta[,1]))[!is.na(unique(as.character(Tarsuslengthdta[,1])))])
    Tarsuslengthdta[,2] = factor(Tarsuslengthdta[,2], levels = unique(as.character(Tarsuslengthdta[,2]))[!is.na(unique(as.character(Tarsuslengthdta[,2])))])
    Tarsuslengthdta[,5] = factor(Tarsuslengthdta[,5], levels = unique(as.character(Tarsuslengthdta[,5]))[!is.na(unique(as.character(Tarsuslengthdta[,5])))])
    Tarsuslengthdta[,6] = factor(Tarsuslengthdta[,6], levels = unique(as.character(Tarsuslengthdta[,6]))[!is.na(unique(as.character(Tarsuslengthdta[,6])))])
    Tarsuslengthdta[,8] = factor(Tarsuslengthdta[,8], levels = unique(as.character(Tarsuslengthdta[,8]))[!is.na(unique(as.character(Tarsuslengthdta[,8])))])

    # as data frame
    Tarsuslengthdta <- as.data.frame(Tarsuslengthdta)

    #subsample individuals we have in the pedigree
    Tarsuslengthdta = Tarsuslengthdta[Tarsuslengthdta[,9] %in% p2[,1],]
    ...
    Tarsuslengthdta = Tarsuslengthdta[Tarsuslengthdta[,4] > 600,]
    ...
    prior <- list(R = list(V = 1, nu = 0.002), # default values of inverse wishart for var
                  G = list(G1 = list(V = 1, nu = 0.5, alpha.V = 1000),
			   G2 = list(V = 1, nu = 0.5, alpha.V = 1000),
			   G3 = list(V = 1, nu = 0.2, alpha.V = 1000),
			   G4 = list(V = 1, nu = 0.2, alpha.V = 1000),
			   G5 = list(V = 1, nu = 0.02, alpha.V = 1000)))

    #Model itself
    TarsuslengthAM <- MCMCglmm(LeftTarsus ~ sex + Rank,
                    random = ~ animal + id + Observer + ClutchIDBorn_Raised + YearBorn,
                    family='gaussian',
		    pedigree = p2,
                    data = Tarsuslengthdta,
                    prior = prior,
                    nitt = 1e6,
                    burnin = 50000,
                    thin=500)
    ...
    save.image("./5_ID_Models/5.2_modelsOutput/AM_TarsusLength_ADULTSRank.RData")
