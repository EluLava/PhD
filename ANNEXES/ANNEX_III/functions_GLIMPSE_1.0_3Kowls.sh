...
### Call SNPs at these positions for all individuals ###
...
REF_SNPs_Calling_perINDV(){
	...
	parallel --rpl '{/..} s:^.*/::;s:_markdup\.bam::;' -j${threads} 'grep "{1/..}" ./inputFILES/3Kowls_samplesFile_INDVSex.tsv > ./inputFILES/TMPlist_{1/..}.tsv' ::: $(find "${BAMpath}" -maxdepth 1 -name "*.bam")

	#Calling the SNPs
	parallel --rpl '{/..} s:^.*/::;s:_markdup\.bam::;' -j${threads} "bcftools mpileup -f ${fastaREF} -I -E -a 'FORMAT/DP' -T \
	/work/FAC/FBM/DEE/jgoudet/barn_owl/Common/3000owls/4_REF_PANEL/data/Out_ShapeIt/RPall502/PerSuperScaffolds/RPAll502_Libnames_Filtered_WhatshapPhased_ShapeitPhased_{2}.vcf.gz -r {2} {1} -O u | bcftools call --ploidy-file ./inputFILES/ploidyFile.txt \
	-S ./inputFILES/TMPlist_{1/..}.tsv -Aim -C alleles -T ./1_SNPcalling_3Kowls/SNPs_RPAll502_Libnames_Filtered_WhatshapPhased_ShapeitPhased_{2}.tsv.gz -O z -o ${outputDIR}/{1/..}_{2}.vcf.gz" \
...
INDVs_VCF_merging(){
	...
	parallel -j${threads} "ls ./1_SNPcalling_3Kowls/*_{}.vcf.gz > ./inputFILES/TMP_{}_INDVs.list" ::: $(grep -v '#' ${CHRlist} | awk '{print $0}')
	...
	#find ./2_SNPcalling_3Kowls -name '*.vcf.gz*' | xargs -I {} bash -c 'rm {}'
...
CHR_chunking(){
	...
	GLIMPSE_chunk=/work/FAC/FBM/DEE/jgoudet/barn_owl/Common/tools/GLIMPSE-glimpse1/chunk/bin/GLIMPSE_chunk
...
CHR_phasing_impute(){
	...
	GLIMPSE_phase=/work/FAC/FBM/DEE/jgoudet/barn_owl/Common/tools/GLIMPSE-glimpse1/phase/bin/GLIMPSE_phase
	...
	do
		...
		${GLIMPSE_phase} --input ${VCF} --reference ${REF} --thread ${threads} --ne ${Ne} --burnin 100 --main 15 --input-region ${IRG} --map /work/FAC/FBM/DEE/jgoudet/barn_owl/Common/3000owls/4_REF_PANEL/data/RecombinationMap/${CHR}.map.txt \
		--samples-file /work/FAC/FBM/DEE/jgoudet/barn_owl/Common/3000owls/2_GLIMPSE/data/inputFILES/GLIMPSEploidyfiles/${CHR}.ploidy --output-region ${ORG} --output ${OUT}
...
CHR_ligating_chunks(){
	...
	GLIMPSE_ligate=/work/FAC/FBM/DEE/jgoudet/barn_owl/Common/tools/GLIMPSE-glimpse1/ligate/bin/GLIMPSE_ligate
...
CHR_sample_haplotypes(){
	...
	CHR=$1
        ...
        #Save shortcut for GLIMPSE_ligate executable
        GLIMPSE_sample=/work/FAC/FBM/DEE/jgoudet/barn_owl/Common/tools/GLIMPSE-glimpse1/sample/bin/GLIMPSE_sample
